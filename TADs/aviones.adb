...
function BuscarPorTipo(L: in TAvion; Tipo: in String) return RegAvion is
   Ptr : TAvion := L;
begin
   ...
   begin
      ...
      while not Vacia(Ptr) loop
         ...
         Put("Campo1 : "); Put(Info(Ptr).Tipo(1..Info(Ptr).LongTipo)); New_Line;
         Put("Campo2 : "); Put(Info(Ptr).Modelo(1..Info(Ptr).LongMod)); New_Line;
         Put_Line("--------------------------------------");
         Ptr := Sig(Ptr);
      end loop;
   end Listar;
   ...
   procedure Guardar(L: in TAvion; Archivo: in String) is
      Arch : aviones.File_Type;
      Ptr  : TAvion := L;
   begin
      aviones.Create(Arch, aviones.Out_File, Archivo);
      while not Vacia(Ptr) loop
         aviones.Write(Arch, Info(Ptr));
         Ptr := Sig(Ptr);
      end loop;
      aviones.Close(Arch);
   end Guardar;
   ...
   begin
      Crear(L);
      aviones.Open(Arch, aviones.In_File, Archivo);
      while not aviones.End_Of_File(Arch) loop
         aviones.Read(Arch, Elem);
         InsertarPorFinal(L, Elem);
      end loop;
      aviones.Close(Arch);
   exception
      when aviones.Name_Error => null;
   end Cargar;
